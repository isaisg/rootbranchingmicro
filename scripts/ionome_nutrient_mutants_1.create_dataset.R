...
df <- read.xlsx(file = "./rawdata/rawdata_ionomics_mutants_2022_03_30_Athaliana_Summary.xlsx",sheetIndex = 3)
df <- df[-1,]
df$ICP.MS.tube <- df$ICP.MS.tube %>%
  gsub(pattern = " ",replacement = "")
colnames(df)[1] <- "UiD"
colnames(df)[2] <- "Medium"

df <- which(!(is.na(df$UiD))) %>%
  df[.,] %>% droplevels

Map <- df[,1:6]
Tab <- df[,7:ncol(df)] %>% as.matrix
...
for(i in 1:ncol(Tab)){
  ...
  max_val <- quantile(Tab[,i],0.95,na.rm = T)
  min_val <- quantile(Tab[,i],0.05,na.rm = T)
...
saveRDS(object = mlist,file = "./cleandata/dat_lr_ionome_nutrient_mutants.RDS")
rm(list=ls())
gc()
